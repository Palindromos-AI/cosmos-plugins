...
The marketplace manifest discovers each plugin through a relative `./plugins/<name>` path. Each plugin manifest then exposes its bundled `skills/` directory. The plugins do not depend on each other and can be installed separately. DingTalk and Feishu collection resolve each user's writable workspace, localized signed-in app, display timezone, and Node executable at run time through Computer Use and publish only locally verified Markdown. `cosmos-stockdata-tools` is self-contained: installed skill metadata resolves its own scripts, while every user supplies an independent SuperMind token from their environment or home-directory configuration and extracted workbooks always remain outside the plugin. Its current runtime remains SuperMind-only; fallback scripts are added only after a confirmed coverage gap and explicit user approval, with baostock preferred over akshare when both can cover the requirement.
