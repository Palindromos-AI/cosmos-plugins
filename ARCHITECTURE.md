...
The marketplace manifest discovers each plugin through a relative `./plugins/<name>` path. Each plugin manifest then exposes its bundled `skills/` directory. The plugins remain independently installable and retain separate `~/.config/<plugin>/runtime.json` bindings, but both workspace-capable plugins use one user-chosen `<cosmos-workspace-root>` with isolated `sources/` and `stockdata/` subtrees. All source reports stay under `sources/output/`; private run evidence remains temporary. DingTalk and Feishu resolve app target, display timezone, and Node executable at run time through Computer Use and admit at most one evidence-backed automatic repair per run. `cosmos-stockdata-tools` contains generic immutable SuperMind transport and contract-neutral references, while executable business code accumulates only under `stockdata/`. Rebinding requires explicit authorization and never moves old data. Marketplace, plugin, and Skill updates own only replaceable capability files: they never invoke configuration or alter external bindings, credentials, settings, or workspace content. Both runtime managers accept only their current versioned schema and fail closed on missing or unsupported schemas. Cross-user capability sharing requires a separate upstream release.
