...
- `llm-wiki`: builds and maintains a persistent, source-grounded wiki of Markdown pages inside an Obsidian vault, following the LLM Wiki pattern — raw notes stay immutable, the model owns every wiki page, and a user-editable schema records the conventions. It ingests notes into source, entity, and concept pages with verbatim provenance; answers questions from the wiki with citations and files answers, comparisons, and syntheses back as analysis pages on request; keeps a deterministic index and an append-only, greppable log; and lints for dead links, orphans, duplicates, stale analyses, ungrounded quotations, and schema drift.
- `raw-to-markdown`: converts user-named files under `raw/` into same-directory Markdown sidecars through a pinned engine, so non-Markdown originals can be ingested.

The Obsidian authoring skills (Obsidian Flavored Markdown, Bases, JSON Canvas, Obsidian CLI) and the `defuddle` web-extraction skill are not packaged here. Install them from the official [kepano/obsidian-skills](https://github.com/kepano/obsidian-skills) collection if you want them. The three investment research Skills—`industry-research`, `investment-research`, and `macro-research`—are intentionally excluded.
...
- `zsxq-fetch`: uses the user's existing signed-in Chrome session to collect topics from one named Knowledge Planet for a requested `Asia/Shanghai` (`UTC+08:00`) calendar date or contiguous date range merged into one report, optionally filtered by a natural-language requirement (every topic by default), extracts topic bodies plus image, linked-page (one hop from the topic, read through the clean-webpage extraction skill with the signed-in Chrome as fallback), PDF, and downloadable HTML and Word content, and writes a reader-facing content-only Markdown archive.
...
- `llm-wiki` writes plain Markdown with normal file tools; Obsidian is how you read the wiki, not a runtime requirement.
...
- `zsxq-fetch` requires Node.js 22.13 or later, the Codex Chrome-control capability, and a Knowledge Planet session already signed in within Chrome. For pages linked from topics it uses an installed clean-webpage extraction skill — `defuddle` from the kepano/obsidian-skills collection — when one is present, and reads through the signed-in Chrome otherwise. Every requested date, date range, and “today” use the Beijing `Asia/Shanghai` boundary rather than the host or browser timezone; a multi-day range is collected day by day and merged into one report. Its deterministic runner uses only built-in Node.js APIs; native-pixel image tiling installs the pinned `sharp` dependency from the skill-local lockfile the first time it is needed, without asking.
...
- `sources-orchestrator` requires Codex subagents (enabled by default) plus whatever each delegated channel skill requires. Subagents inherit the conversation's skills, sandbox, and permission mode; approval prompts raised by a subagent surface in the same conversation. Running several Knowledge Planets in parallel needs no extra setup: the tasks share the one signed-in Chrome, each subagent in its own browser session.
