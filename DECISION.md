...
- **Clarification:** `stockdata-fetch` must not carry a predetermined full-market extractor, fixed workbook, universal schema, speculative field set, or mutable user scripts. Installed plugin directories are versioned caches and are not durable shared state.
...
- **Decision:** Keep the distributed Skill instruction-only. On first use, require each user to explicitly identify a durable external `<stockdata-workspace>` and persist that binding outside the plugin through `STOCKDATA_WORKSPACE` or `~/.config/cosmos-stockdata-tools/workspace`; later tasks must verify and reuse it instead of inferring from the current directory. Rebinding or migration requires explicit authorization and must preserve the old workspace. Create the smallest reusable scripts and tests there, then extend the same workspace while preserving accepted capabilities. Marketplace updates must not write to this workspace. Choose sources at field or dataset level in the order `SuperMind -> baostock -> AKShare`. Share a capability across installations only through a separate authorized upstream review and versioned release.
- **Context:** This supersedes the fixed SuperMind program, workbook contract, driver lifecycle, thresholds, token-resolution defaults, and skill-local accumulation decisions. User workspaces are independent by default; credentials and private data remain external and never enter a marketplace release.
