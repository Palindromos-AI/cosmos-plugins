...
test("CLS writer creates fresh output exclusively and refuses foreign files", async (t) => {
  const context = await fixture(t);
  const workspaceRoot = path.join(context.root, "cosmos-workspace");
  await mkdir(workspaceRoot);
  const binding = await configureRuntime({ workspaceRoot, ...context });
  const outputDirectory = path.join(binding.workspace, "output", "cls");
  const target = path.join(outputDirectory, "report.md");

  const written = await writeMarkdown(target, "content\n", { runtimeOptions: context });
  assert.equal(written, target);
  assert.equal(await readFile(target, "utf8"), "content\n");
  assert.deepEqual(await readdir(outputDirectory), ["report.md"]);

  const foreign = path.join(outputDirectory, "foreign.md");
  await writeFile(foreign, "user file\n");
  await assert.rejects(
    writeMarkdown(foreign, "content\n", { runtimeOptions: context }),
    /not generated by cls-fetch/,
  );
  assert.equal(await readFile(foreign, "utf8"), "user file\n");
  assert.deepEqual(
    (await readdir(outputDirectory)).sort(),
    ["foreign.md", "report.md"],
  );
});
