...
// `repo` defaults to the pinned repository, so only these are required.
const REQUIRED_CLI_ARGUMENTS = new Set(["plugin", "scope"]);
...
export async function writeReport({
  repo = pinnedFixReportRepository().reportRepository,
  plugin,
  scope,
  content,
...
export function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!flag?.startsWith("--") || value === undefined) {
      throw new Error("expected --plugin and --scope argument pairs");
    }
  ...
  for (const name of REQUIRED_CLI_ARGUMENTS) {
    if (!Object.hasOwn(values, name)) {
