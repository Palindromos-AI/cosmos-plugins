...
export async function runPreflight({ repo }) {
  return preflight(repo);
}

export async function runPush({ repo }) {
...
export async function runPublish({ repo, report }) {
  const canonical = await requireReportRepository(repo);
  const upstream = resolveUpstream(canonical);
...
function parseArguments(argv) {
  ...
  if (!values.repo) {
    throw new PublishError("missing argument: --repo");
  }
