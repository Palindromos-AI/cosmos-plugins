...
The single active DOM contract is `ZSXQ_BROWSER_CONTRACT` (`zsxq-web-angular-v11`). A live DOM change is a repair: tell the user the confirmed cause and the smallest intended change, and change the contract only after their explicit approval, bumping the version string.
