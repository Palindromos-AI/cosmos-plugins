...
Create a private temporary parent directory, choose a child path that does not yet exist, and let `init` atomically create that run workspace. Requires Node.js 22.13 or later, the first 22.x release where the built-in SQLite API used for crash-safe process locking no longer needs an experimental startup flag.
