...
- `downloadZsxqTimelineFileOnTab(tab, target)` / `downloadZsxqDetailFileOnTab(tab, target)` — official member download for one inventoried PDF, HTML, or Word file card. Map the `file_download_targets` entry onto the camelCase inputs: `type` as `expectedType`, `filename` as `expectedFilename`, `file_ordinal` as `fileOrdinal`, `expected_file_platform_id` as `expectedFilePlatformId`, plus `dom_topic_index` as `topicDomIndex` for the timeline downloader or `topic_id` as `expectedTopicId` for the detail downloader. The helper selects the card by its stable `data-file-id`, requires one visible `下载文件` control whose preview name equals the expected filename, arms the browser `download` event before clicking, and returns the completed download.
...
The single active DOM contract is `ZSXQ_BROWSER_CONTRACT` (`zsxq-web-angular-v10`). A live DOM change is a repair: tell the user the confirmed cause and the smallest intended change, and change the contract only after their explicit approval, bumping the version string.
