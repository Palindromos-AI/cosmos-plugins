---
name: dingding-fetch
description: Read all messages from exact user-specified DingTalk (钉钉) group chats/群聊 on today, one requested historical Beijing calendar date, or one contiguous Beijing date range of at most 31 days, optionally select messages matching a user-supplied information requirement (all in-window messages by default), extract unchanged text and substantive image text, and write a content-only Markdown (.md) snapshot. Use when the user invokes $dingding-fetch or asks to read, filter, extract, archive, or 汇总 one or more DingTalk groups' 当天/今日、指定历史日期或日期区间消息. Do not use for future dates, sending messages, scheduled runs, non-group conversations, or unattended background monitoring.
---
