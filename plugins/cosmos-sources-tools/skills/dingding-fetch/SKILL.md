...
- Treat DingTalk's displayed timestamps as app-local display values. Resolve their date from the nearest date separator or message timestamp, interpret them in `<display-timezone>`, convert the represented timestamp to an instant, and then convert that instant to Beijing time before testing the window.
...
- Exclude unrelated messages, reactions, read receipts, typing indicators, group notices unrelated to the requested information, and interface labels. Preserve a quoted or replied-to message only when it is displayed as part of the selected source message.
