...
- The one repair attempt is shared across all groups, threads, and phases. A successful repair does not reset it. A corrupt, missing-after-use, replaced, or conflicting state file is a direct blocker, never permission to recreate the budget.
...
- Treat Feishu's displayed timestamps as Beijing time (`Asia/Shanghai`). Resolve each message's date from the nearest date separator or message timestamp; no timezone conversion is needed. If the app visibly uses a non-Beijing timezone, stop and report it.
