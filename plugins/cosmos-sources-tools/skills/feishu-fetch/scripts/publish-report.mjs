...
export async function publishReport({
  ...
  if (!/^\.feishu-digest-[A-Za-z0-9-]+\.tmp$/.test(path.basename(draft))) {
    throw new Error("draft must use the .feishu-digest-<id>.tmp name");
  }
  ...
  const stage = path.join(
    path.dirname(target),
    `.feishu-publish-${randomBytes(12).toString("hex")}.tmp`,
  );
