...
| `last_time` | Exclusive Unix-second cursor; one walk covers a whole multi-day window |
...
- Start at the earlier of the current Unix second plus one and the window's exclusive end (the Shanghai midnight after the requested date, or after `--end-date` for a range).
...
- Stop only after a page crosses the window's Shanghai start boundary.
- Treat an empty page before crossing that boundary as an incomplete-source failure.
