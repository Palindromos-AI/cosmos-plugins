...
export function dateWindow(dateString, endDateString = dateString) {
  const start = dayStartSeconds(dateString);
  const endStart = dayStartSeconds(endDateString);
  if (endStart < start) {
    throw new ClsFetchError(
      `Range start ${dateString} must not be later than end ${endDateString}`,
    );
  }
  const days = Math.round((endStart - start) / 86_400) + 1;
  if (days > MAX_RANGE_DAYS) {
    throw new ClsFetchError(`Date range must span at most ${MAX_RANGE_DAYS} days`);
  }
...
export async function fetchDay({
  ...
  const { start, end, days } = dateWindow(date, lastDate);
  const pageBudget = maxPages ?? 500 * days;
  if (pageSize > MAX_PAGE_SIZE) {
    throw new ClsFetchError(`pageSize cannot exceed ${MAX_PAGE_SIZE}`);
  }
  let cursor = initialCursor(lastDate, now);
  let pages = 0;
  let stopReason = null;
  ...
  if (!stopReason) {
    throw new ClsFetchError(`Pagination exceeded maxPages=${pageBudget} before reaching midnight`);
  }
  ...
  return {
    ...
    timezone: TIME_ZONE,
    snapshot_at:
      lastDate === formatShanghaiDate(now)
        ? now.toISOString()
...
export function parseArgs(argv) {
  ...
  if (options["end-date"] && !options.date) {
    throw new ClsFetchError("--end-date requires --date");
  }
  return {
